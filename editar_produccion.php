...
if(isset($_GET['id'])) {
    ...
    $query = "SELECT * FROM produccion_t_s WHERE id_produccion = ?";
    $stmt = $conn->prepare($query);
    $stmt->bind_param("i", $produccion_id);
    $stmt->execute();
...
$stmt_profesores_completo->bind_param("i", $produccion_id);
...
<body>
<div class="container">
    <h4 class="mb-4">Editar Produccion</h4>
    <form action="actualizar_solicitud_produccion.php" method="post">
        ...
        <input type="hidden" name="id_solicitud" value="<?= $produccion_id ?>">

        <!-- Otros campos -->
        <div class="row mb-3">
            <div class="col-md-2">
                <label for="identificador" class="form-label">Identificador:</label>
                <input type="text" class="form-control" id="identificador" name="identificador" value="<?= $produccion['identificador'] ?>" required>
            </div>
            <div class="col-md-2">
                <label for="oficio" class="form-label">Oficio:</label>
                <input type="text" class="form-control" id="oficio" name="oficio" value="<?= $produccion['numero_oficio'] ?>" required>
            </div>
            <div class="col-md-2">
                <label for="productop" class="form-label">Producto:</label>
                <input type="text" class="form-control" id="productop" name="productop" value="<?= $produccion['productop'] ?>" required>
            </div>
            <div class="col-md-2">
                <label for="numero_profesores" class="form-label">Num. Profesores:</label>
                <input type="text" class="form-control" id="numero_profesores" name="numero_profesores" value="<?= $produccion['numero_profesores'] ?>" required>
            </div>
        </div>
        ...
        <div class="row mb-3">
            <div class="col-md-3">
                ...
                <input type="text" class="form-control" id="puntaje" name="puntaje" value="<?= $produccion['puntaje'] ?>" required>
            </div>
        </div>
