...
$numero_profesores = isset($_POST['hidden_numero_profesores']) ? $_POST['hidden_numero_profesores'] : null;
    ...
    // Crear la consulta de inserción para la tabla `innovacion`
    $sql = "INSERT INTO innovacion (
                identificador,
                numero_oficio,
                fecha_solicitud,
                producto, impacto,
    ...
    if ($stmt = $conn->prepare($sql)) {
        ...
        if ($stmt->execute()) {
            $id_innovacion = $conn->insert_id;
            for ($i = 1; $i <= $numero_profesores; $i++) {
                $id_profesor = trim($_POST["documento_$i"]);
                if (!empty($id_profesor)) {
                    $sql_profesor = "INSERT INTO innovacion_profesor (id_innovacion, id_profesor) VALUES (?, ?)";
                    if ($stmt_profesor = $conn->prepare($sql_profesor)) {
                        $stmt_profesor->bind_param("is", $id_innovacion, $id_profesor);
            ...
            echo "innovacion y profesores guardados exitosamente.";
        } else {
            error_log("Error al guardar la innovacion: " . $stmt->error);
            echo "Error al guardar la innovacion.";
        }
        $stmt->close();
    } else {
