 ...
 * Versión Inteligente: Consolida artículos múltiples por docente Y agrupa coautores por Facultad.
 * Incluye tamaño Oficio, Encabezado, Pie de página, Nombre Propio en Departamentos 
 * y detección automática de GÉNERO (F/M) para gramática dinámica.
 */
require 'conn.php';
...
// --- 2. CONSULTA PLANA (Añadido t.sexo) ---
$sql = "
    SELECT 
...
while ($row = $res->fetch_assoc()) {
    ...
    if (!in_array($id_art, $seen_articles[$cc])) {
        $prof_records[$cc][] = $row;
...
foreach ($prof_records as $cc => $arts) {
    if (count($arts) > 1) {
        $prof_multiples[$cc] = $arts;
    } else {
        $row = $arts[0];
        $id_art = $row['id_solicitud_articulo'];
        $fac = $row['facultad'];
        
        if (!isset($articulos_grupales[$id_art])) $articulos_grupales[$id_art] = [];
        if (!isset($articulos_grupales[$id_art][$fac])) $articulos_grupales[$id_art][$fac] = [];
...
foreach ($prof_multiples as $cc => $arts) {
    $docentes_list = [[
        'documento_tercero' => $cc,
        'profe_nombre' => $arts[0]['profe_nombre'],
        'sexo' => $arts[0]['sexo'], /* Pasamos el sexo al array */
        'email' => $arts[0]['email'],
        'departamento' => $arts[0]['departamento']
    ]];
    generarResolucion($phpWord, $docentes_list, $arts, $arts[0]['facultad'], $styleTable);
}
    ...
    foreach ($facultades as $fac => $profesores) {
        ...
        foreach ($profesores as $p) {
            $docentes_list[] = [
                ...
                'sexo' => $p['sexo'], /* Pasamos el sexo al array */
                'email' => $p['email'],
                'departamento' => $p['departamento']
            ];
        }
        $articulos_list = [$profesores[0]]; 
        generarResolucion($phpWord, $docentes_list, $articulos_list, $fac, $styleTable);
    }
}

// --- 6. FUNCIÓN DE RENDERIZADO ---
function generarResolucion($phpWord, $docentes_list, $articulos_list, $facultad, $styleTable) {
    ...
    // PREPARACIÓN DE DATOS (Artículos)
    $conceptos = []; $literales = []; $tiposArray = []; $oficios = []; $puntajeTotal = 0;
    foreach ($articulos_list as $art) {
        $puntajeTotal += floatval($art['puntaje']);
    ...
    $allFemale = true; // Asumimos que todas son mujeres hasta que se demuestre lo contrario
    ...
    $section->addText($lblDepto . mb_strtoupper($textoDeptos, 'UTF-8'), 'StyleBold'); /* Aquí sigue en mayúscula como subtítulo */
