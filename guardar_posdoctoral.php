...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
   ...
   $sql = "INSERT INTO posdoctoral (identificador, numero_oficio, documento_profesor, titulo_obtenido,   institucion, fecha_terminacion,  puntaje, tipo_productividad)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)";

$stmt = $conn->prepare($sql);

// Asegúrate de que el tipo de cada parámetro coincida con el tipo de datos en la tabla
$stmt->bind_param("ssssssss", $identificador, $inputTrdFac, $documento_profesor, $producto,   $institucion, $fecha_terminacion,  $puntaje, $tipo_productividad);
...
if ($stmt->execute()) {
    ...
    $sql_titulo_profesor = "INSERT INTO posdoctoral_profesor (id_titulo, fk_tercero) VALUES (?, ?)";
