...
if(isset($_GET['id'])) {
    $innovacion_id = $_GET['id'];

    // Consulta para obtener los datos de la comisión basada en el id
    $query = "SELECT * FROM innovacion WHERE id_innovacion = ?";
    $stmt = $conn->prepare($query);
    $stmt->bind_param("i", $innovacion_id);
    $stmt->execute();
    $result = $stmt->get_result();
    $innovacion = $result->fetch_assoc();
...
<div class="container">
    <h4 class="mb-4">Editar Innovacion</h4>
    <form action="actualizar_solicitud_innovacion.php" method="post">
...
<input type="hidden" name="id_solicitud" value="<?= $innovacion_id ?>">
        <div class="col-md-12 form-container">
            <div id="profesores-container" class="form-control">
        ...
        <div class="row mb-3">
            <div class="col-md-2">
                ...
                <input type="text" class="form-control" id="identificador" name="identificador" value="<?= $innovacion['identificador'] ?>" required>
            </div>
            <div class="col-md-2">
                <label for="oficio" class="form-label">Oficio:</label>
                <input type="text" class="form-control" id="oficio" name="oficio" value="<?= $innovacion['numero_oficio'] ?>" required>
            </div>
                <div class="col-md-2">
                <label for="tipo_creacion" class="form-label">Impacto:</label>
<select class="form-control" id="tipo_creacion" name="impacto" required>
    <option value="INTERNACIONAL" <?= ($innovacion['impacto'] == 'INTERNACIONAL') ? 'selected' : '' ?>>Internacional</option>
    <option value="NACIONAL" <?= ($innovacion['impacto'] == 'NACIONAL') ? 'selected' : '' ?>>Nacional</option>
</select>
            </div>
         <div class="col-md-2">
                <label for="producto" class="form-label">producto:</label>
                <input type="text" class="form-control" id="producto" name="producto" value="<?= $innovacion['producto'] ?>" required>
            </div>
             <div class="col-md-2">
                <label for="numero_profesores" class="form-label">num profesores:</label>
                <input type="text" class="form-control" id="numero_profesores" name="numero_profesores" value="<?= $innovacion['numero_profesores'] ?>" required>
            </div>     
        </div>
         ...
         <div class="row mb-3">
             ...
             <div class="col-md-3">
                ...
                <input type="text" class="form-control" id="puntaje" name="puntaje" value="<?= $innovacion['puntaje'] ?>" required>
            </div>
        </div>
