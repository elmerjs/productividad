...
$numero_profesores = isset($_POST['hidden_numero_profesores']) ? $_POST['hidden_numero_profesores'] : null;
    ...
    // Crear la consulta de inserción para la tabla `proudccion`
    $sql = "INSERT INTO produccion_t_s (
                identificador,
                numero_oficio,
                fecha_solicitud,
                productop, 
        ...
        if ($stmt->execute()) {
            ...
            for ($i = 1; $i <= $numero_profesores; $i++) {
                ...
                if (!empty($id_profesor)) {
                    $sql_profesor = "INSERT INTO produccionp_profesor (id_produccion, id_profesor) VALUES (?, ?)";
            ...
            echo "produccion y profesores guardados exitosamente.";
        } else {
            error_log("Error al guardar la proudccion: " . $stmt->error);
            echo "Error al guardar la produccion.";
        }
        $stmt->close();
    } else {
