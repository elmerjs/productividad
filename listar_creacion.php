...
// Agrupar los resultados y ordenar del más reciente al más antiguo
$sql .= " GROUP BY 
    t.id, t.numeroOficio, t.fecha_solicitud, t.tipo_producto, t.identificador_completo, t.impacto, t.producto, t.nombre_evento, t.evento, t.fecha_evento, t.lugar_evento, t.autores, t.evaluacion1, t.evaluacion2, t.puntaje_final, t.estado_creacion
ORDER BY t.id DESC";
...
$identificadores_result = $conn->query("SELECT DISTINCT identificador_completo FROM creacion WHERE identificador_completo IS NOT NULL ORDER BY identificador_completo DESC"); 
...
$ultimos_lotes_result = $conn->query("SELECT DISTINCT identificador_completo FROM creacion WHERE identificador_completo IS NOT NULL AND identificador_completo != '' ORDER BY identificador_completo DESC LIMIT 6");
$ultimos_lotes = [];
...
<head>
    ...
    <style>
        ...
        #creacion { margin-bottom: 0 !important; }
        #creacion thead th {
            background-color: #f8fafc;
            color: #475569;
...
<div class="module-wrapper">
    ...
    <div class="page-header-inner d-flex flex-wrap justify-content-between align-items-center gap-3">
        <div>
            ...
            <p class="page-subtitle-inner">Listado maestro de creaciones, obras y productos artísticos</p>
        </div>
        
        <div class="d-flex gap-2 flex-wrap">
            <button id="openModalcr" class="btn-modern btn-m-xls">
                <i class="fas fa-file-excel"></i> Exportar XLS
            </button>
            <button id="openModalCuadroscr" class="btn-modern btn-m-cuadros">
                <i class="fas fa-table"></i> Generar Cuadros
            </button>
            <button id="openModalResolucionescr" class="btn-modern btn-m-res">
    ...
    <div class="table-responsive">
        <table id="creacion" class="table table-hover align-middle" data-order='[[ 0, "desc" ]]' style="width:100%">
            <thead>
                <tr> 
            ...
            <tbody>
                ...
                while ($row = $result->fetch_assoc()) {
                    ...
                    $tipo_prod = $row['tipo_producto'] ?? 'N/A';
                    
                    // LÓGICA DE ESTADOS MINIMALISTAS
                    $estadoOriginal = strtolower(trim($row['estado_creacion'] ?? ''));
                    ...
                    echo '<td><span class="badge bg-light text-secondary border px-1">' . htmlspecialchars($tipo_prod) . '</span></td>';
                    ...
                    echo '<a href="editar_creacion_art.php?id=' . $row['id'] . '" class="btn btn-light border btn-action text-primary shadow-sm" title="Editar"><i class="fas fa-pen"></i></a> ';
                    echo '<button class="delete-btn btn btn-light border btn-action text-danger shadow-sm" data-id="' . $row['id'] . '" title="Eliminar"><i class="fas fa-trash-alt"></i></button>';
                    echo '</td>';
                    echo '</tr>';
                }
                ?>
    ...
    <div class="quick-audit-section">
        <div class="quick-audit-title">
            <i class="fas fa-folder-tree text-secondary me-1"></i> Auditoría Rápida de Lotes (Creación Artística)
        </div>
        <div class="lotes-carousel">
            <?php foreach($ultimos_lotes as $lote): ?>
                <a href="auditoria_lote.php?lote=<?php echo urlencode($lote); ?>" class="lote-card" title="Ver lote: <?php echo htmlspecialchars($lote); ?>">
                    <i class="fas fa-palette text-warning"></i> <?php echo htmlspecialchars($lote); ?>
                </a>
            <?php endforeach; ?>
        </div>
    </div>
...
<div id="modalcr" class="modal">
    <div class="modal-content">
        <div class="d-flex justify-content-between align-items-center mb-4">
             <h4 class="fw-bold text-success m-0"><i class="fas fa-file-excel me-2"></i>Reporte XLS Creación</h4>
             <span class="close close-xls">&times;</span>
        </div>
        <div class="modal-body p-0">
            <form action="report_creacion.php" method="GET">
                <div class="mb-3">
                    <label for="ano_xls_cr" class="form-label text-secondary fw-semibold">Año:</label>
                    <select name="ano" id="ano_xls_cr" class="form-select">
                        <option value="">Todos los años...</option>
                        <?php foreach($unique_years as $y) echo "<option value='$y'>$y</option>"; ?>
                    </select>
                </div>
                <div class="mb-4">
                    <label for="identificador_solicitud_cr" class="form-label text-secondary fw-semibold">Identificador de Solicitud:</label>
                    <select name="identificador_solicitud" id="identificador_solicitud_cr" class="form-select">
    ...
    <div class="modal-content">
        ...
        <div class="modal-body p-0">
            <form action="cuadros_creacion.php" method="GET">
                <div class="mb-3">
                    <label for="ano_cuadros_cr" class="form-label text-secondary fw-semibold">Año:</label>
                    <select name="cuadro_ano" id="ano_cuadros_cr" class="form-select">
                        <option value="">Todos los años...</option>
                        <?php foreach($unique_years as $y) echo "<option value='$y'>$y</option>"; ?>
                    </select>
                </div>
                <div class="mb-4">
                    <label for="cuadro_identificador_solicitud_cr" class="form-label text-secondary fw-semibold">Identificador de Solicitud:</label>
                    <select name="cuadro_identificador_solicitud" id="cuadro_identificador_solicitud_cr" class="form-select">
                        <option value="">Todos los paquetes...</option>
                        <?php
                        foreach ($identificadores as $id) {
                            echo '<option value="' . htmlspecialchars($id) . '" data-ano="' . substr($id, 0, 4) . '">' . htmlspecialchars($id) . '</option>';
...
<div id="modalResolucionescr" class="modal">
    <div class="modal-content" style="max-width: 650px;">
        <div class="d-flex justify-content-between align-items-center mb-4">
             <h4 class="fw-bold text-info m-0" style="color: #0284c7 !important;"><i class="fas fa-file-word me-2"></i>Resoluciones Creación Artística</h4>
             <span class="close close-resoluciones-cr">&times;</span>
        </div>
        <div class="modal-body p-0">
            <form action="resoluciones_creacion.php" method="GET">
                ...
                <div class="row bg-light p-3 mb-4 border rounded-3 mx-0">
                    <div class="col-md-6 mb-2">
                        <label for="ano_res_cr" class="form-label text-secondary fw-semibold">Filtro por Año:</label>
                        <select id="ano_res_cr" class="form-select">
                            <option value="">Seleccione un año...</option>
                            <?php foreach($unique_years as $y) echo "<option value='$y'>$y</option>"; ?>
                        </select>
                    </div>
                    <div class="col-md-6 mb-2">
                        <label for="cuadro_identificador_creacion" class="form-label text-secondary fw-semibold">Identificador (Paquete):</label>
                        <select name="cuadro_identificador_creacion" id="cuadro_identificador_creacion" class="form-select" required>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#openModalcr').on('click', function() { $('#modalcr').fadeIn(200); });
        $('#openModalCuadroscr').on('click', function() { $('#modalCuadroscr').fadeIn(200); });
        $('#openModalResolucionescr').on('click', function() { $('#modalResolucionescr').fadeIn(200); }); 
        
        $('.close-xls').on('click', function() { $('#modalcr').fadeOut(200); });
        $('.close-cuadros').on('click', function() { $('#modalCuadroscr').fadeOut(200); });
        $('.close-resoluciones-cr').on('click', function() { $('#modalResolucionescr').fadeOut(200); });
        ...
        $('#ano_xls_cr').on('change', function() { applyYearFilter('ano_xls_cr', 'identificador_solicitud_cr'); });
        $('#ano_cuadros_cr').on('change', function() { applyYearFilter('ano_cuadros_cr', 'cuadro_identificador_solicitud_cr'); });
        $('#ano_res_cr').on('change', function() { applyYearFilter('ano_res_cr', 'cuadro_identificador_creacion'); });

        // Función AJAX para eliminar con motivo (Respetando el código original)
            ...
            if (confirm("¿Estás seguro de que quieres eliminar esta solicitud?")) {
                ...
                if (motivo && motivo.trim() !== "") {
                    $.ajax({
                        ...
                        type: 'POST',
                        data: { id: id, motivo: motivo }, // Se añade el motivo a la petición POST
                        dataType: 'json',
