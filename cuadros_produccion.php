...
// Crear la consulta SQL para obtener los datos de producción con facultad y departamento
$sql = "
SELECT 
    p.id_produccion,
    p.identificador,
    ...
    GROUP_CONCAT(DISTINCT CONCAT(pr.nombre_completo, ' - ', pr.documento_tercero) 
        ORDER BY pr.documento_tercero SEPARATOR '\n') AS `DETALLES_PROFESORES`
FROM 
    produccion_t_s p
JOIN 
    produccionp_profesor pp ON pp.id_produccion = p.id_produccion
JOIN 
    tercero pr ON pp.id_profesor = pr.documento_tercero
JOIN 
    deparmanentos d ON pr.fk_depto = d.pk_depto
JOIN 
    facultad f ON f.PK_FAC = d.FK_FAC
     WHERE (p.estado is null or p.estado <> 'an')
";
...
$section->addText("INFORME DE PRODUCCIÓN ACADÉMICA", ['bold' => true, 'size' => 14], ['alignment' => Jc::CENTER]);
...
// Imprimir los datos de producción
if ($result->num_rows > 0) {
    while ($data = $result->fetch_assoc()) {
        ...
        $table->addCell(6000)->addText($data['productop'], $cellTextStyle, $paragraphStyle);
        ...
        $table->addCell(6000)->addText($data['tipo_productividad'] ?? 'N/A', $cellTextStyle, $paragraphStyle);
...
$fileName = "Cuadros_Produccion_Academica{$identificador_solicitud}.docx";
