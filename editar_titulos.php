...
$sql_profesores_completo = "
    ...
    FROM titulo_profesor p 
    JOIN tercero ON tercero.documento_tercero = p.fk_tercero
    WHERE p.id_titulo = ?";
$stmt_profesores_completo = $conn->prepare($sql_profesores_completo);
$stmt_profesores_completo->bind_param("i", $titulo_id);
...
<div class="container">
    <h4 class="mb-4">Editar Títulos</h4>
    <form action="actualizar_solicitud_titulo.php" method="post">
...
<input type="hidden" name="id_solicitud" value="<?= $titulo_id ?>">
        <div class="col-md-12 form-container">
            <div id="profesores-container" class="form-control">
        ...
        <div class="row mb-3">
            <div class="col-md-2">
                ...
                <input type="text" class="form-control" id="identificador" name="identificador" value="<?= $titulo['identificador'] ?>" required>
            </div>
            <div class="col-md-2">
                <label for="oficio" class="form-label">Oficio:</label>
                <input type="text" class="form-control" id="oficio" name="oficio" value="<?= $titulo['numero_oficio'] ?>" required>
            </div>
             <div class="col-md-2">
                <label for="oficio" class="form-label">Doc Profesor:</label>
                <input type="text" class="form-control" id="documento_profesor" name="documento_profesor" value="<?= $titulo['documento_profesor'] ?>" required>
            </div>
             <div class="col-md-2">
                <label for="titulo_obtenido" class="form-label">Título:</label>
                <input type="text" class="form-control" id="titulo_obtenido" name="titulo_obtenido" value="<?= $titulo['titulo_obtenido'] ?>" required>
            </div>
                <div class="col-md-2">
                <label for="tipo" class="form-label">tipo:</label>
<select class="form-control" id="tipo" name="tipo" required>
    <option value="EXTERIOR" <?= ($titulo['tipo'] == 'EXTERIOR') ? 'selected' : '' ?>>Internacional</option>
    <option value="NACIONAL" <?= ($titulo['tipo'] == 'NACIONAL') ? 'selected' : '' ?>>Nacional</option>
</select>
            </div>
         <div class="col-md-2">
                <label for="tipo_estudio" class="form-label">tipo_estudio:</label>
                <select class="form-control" id="tipo_estudio" name="tipo_estudio" required>
    <option value="DOCTORADO" <?= ($titulo['tipo_estudio'] == 'DOCTORADO') ? 'selected' : '' ?>>DOCTORADO</option>
    <option value="MAESTRIA" <?= ($titulo['tipo_estudio'] == 'MAESTRIA') ? 'selected' : '' ?>>MAESTRÍA</option>
    <option value="ESPECIALIZACION" <?= ($titulo['tipo_estudio'] == 'ESPECIALIZACION') ? 'selected' : '' ?>>ESPECIALIZACIÓN</option>
</select>
            </div>
             <div class="col-md-2">
                <label for="institucion" class="form-label">institucion:</label>
                <input type="text" class="form-control" id="institucion" name="institucion" value="<?= $titulo['institucion'] ?>" required>
            </div>  
              <div class="col-md-2">
                <label for="fecha_terminacion" class="form-label">fecha_terminacion:</label>
                <input type="text" class="form-control" id="fecha_terminacion" name="fecha_terminacion" value="<?= $titulo['fecha_terminacion'] ?>" required>
            </div>  
             <div class="col-md-2">
                <label for="convalidacion" class="form-label">convalidacion:</label>
                <input type="text" class="form-control" id="convalidacion" name="convalidacion" value="<?= $titulo['resolucion_convalidacion'] ?>" required>
            </div>  
        </div>
         ...
         <div class="row mb-3">
             ...
             <div class="col-md-3">
                ...
                <input type="text" class="form-control" id="puntaje" name="puntaje" value="<?= $titulo['puntaje'] ?>" required>
            </div>
        </div>
