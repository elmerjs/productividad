...
$sql .= " GROUP BY 
    t.id_traduccion,  t.numero_oficio, t.fecha_solicitud, t.producto, t.numero_profesores, t.puntaje, t.estado, t.tipo_productividad
ORDER BY 
...
$identificadores_result = $conn->query("SELECT DISTINCT identificador FROM traduccion_libros"); // Reemplaza con tu consulta original
$identificadores = [];
...
<head>
    ...
    <title>Listado de traduccion libros</title>
...
<div class="container-fluid mt-4">
    <h1>Listado de Traducción Libros</h1>

    <!-- Botones de acciones -->
    <div class="mb-3">
        <button id="openModaltl" class="btn btn-primary">Generar XLS</button>
        <button id="openModalCuadrostl" class="btn btn-secondary">Generar Cuadros</button>
    </div>

    <!-- Tabla donde se mostrarán los datos -->
    <table id="traduccion" class="display table table-striped table-bordered">
        <thead>
            <tr> <th>ID</th>         
                <th>IDENTIFICADOR</th>
                <th>DEPARTAMENTO</th>
        ...
        <tbody>
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                echo '<td>' . htmlspecialchars($row['id_traduccion']) . '</td>';
 ...
 echo '<td class="facultad-truncate" title="' 
               ...
               echo '<td>';
echo '<a href="editar_traduccion.php?id=' . $row['id_traduccion'] . '" class="btn btn-warning btn-sm">Editar</a>';
echo '<button class="btn btn-danger btn-sm" onclick="confirmDeleteWithReason(' . $row['id_traduccion'] . ')">Eliminar</button>';
                echo '</td>';
                echo '</tr>';
            }
            ?>
...
<script>
    $(document).ready(function() {
        ...
        $("#openModaltl").click(function() {
            $("#modaltl").css("display", "block");
        });

        // Abrir el modal para generar Cuadros
        $("#openModalCuadrostl").click(function() {
            $("#modalCuadrostl").css("display", "block");
        });
        ...
        $(".close-cuadros").click(function() {
            $("#modalCuadrostl").css("display", "none");
        });

        // Cerrar los modales si se hace clic fuera de ellos
        $(window).click(function(event) {
            if ($(event.target).is("#modal")) {
                $("#modaltl").css("display", "none");
            }
