...
if(isset($_GET['id'])) {
    $traduccion_id = $_GET['id'];

    // Consulta para obtener los datos de la comisión basada en el id
    $query = "SELECT * FROM traduccion_libros WHERE id_traduccion = ?";
    $stmt = $conn->prepare($query);
    $stmt->bind_param("i", $traduccion_id);
    $stmt->execute();
    $result = $stmt->get_result();
    $traduccion = $result->fetch_assoc();
...
<head>
    ...
    <title>Editar Solicitud traduccion</title>
    <!-- Bootstrap 5 CSS -->
...
<div class="container">
    <h4 class="mb-4">Editar Solicitud traduccion</h4>
    <form action="actualizar_solicitud_traduccion.php" method="post">
...
<input type="hidden" name="id_solicitud" value="<?= $traduccion_id ?>">
        <div class="col-md-12 form-container">
            <div id="profesores-container" class="form-control">
        ...
        <div class="row mb-3">
            <div class="col-md-2">
                ...
                <input type="text" class="form-control" id="identificador" name="identificador" value="<?= $traduccion['identificador'] ?>" required>
            </div>
            <div class="col-md-2">
                <label for="oficio" class="form-label">Oficio:</label>
                <input type="text" class="form-control" id="oficio" name="oficio" value="<?= $traduccion['numero_oficio'] ?>" required>
            </div>
                        <div class="col-md-8">
                <label for="producto" class="form-label">producto:</label>
                <input type="text" class="form-control" id="producto" name="producto" value="<?= $traduccion['producto'] ?>" required>
            </div>
        </div>
  <div class="row mb-3">
          <div class="col-md-3">
                <label for="numero_profesores" class="form-label">numero profesores:</label>
                <input type="text" class="form-control" id="numero_profesores" name="numero_profesores" value="<?= $traduccion['numero_profesores'] ?>" required>
            </div>
         <div class="col-md-3">
                <label for="tipo_traduccion" class="form-label">tipo traduccion:</label>
                <input type="text" class="form-control" id="tipo_traduccion" name="tipo_traduccion" value="<?= $traduccion['tipo_traduccion'] ?>" required>
            </div>
             <div class="col-md-3">
                <label for="puntaje" class="form-label">Puntaje:</label>
                <input type="text" class="form-control" id="puntaje" name="puntaje" value="<?= $traduccion['puntaje'] ?>" required>
            </div>
        </div>
