...
$sql = "
    SELECT 
        ...
        t.num_resolucion,
        t.fecha_resolucion,
        t.nombre_vicerrector,
        t.genero_vicerrector,
        t.nombre_reviso,
        t.nombre_elaboro,
...
$sql .= " 
    GROUP BY 
        ...
        t.numero_profesores, t.puntaje, t.estado, t.tipo_productividad, 
        t.num_resolucion, t.fecha_resolucion, t.nombre_vicerrector, 
        t.genero_vicerrector, t.nombre_reviso, t.nombre_elaboro
    ORDER BY 
        t.id DESC;
";
...
if (!$result) {
    echo "<div class='alert alert-danger m-3'>Error SQL en Traducción Bonificación: " . $conn->error . "</div>";
}

// Obtener los identificadores y extraer años para modales
$identificadores_result = $conn->query("SELECT DISTINCT identificador FROM traduccion_bon WHERE identificador IS NOT NULL ORDER BY identificador DESC");
$identificadores = [];
$unique_years = [];
...
$ultimos_lotes_result = $conn->query("SELECT DISTINCT identificador FROM traduccion_bon WHERE identificador IS NOT NULL AND identificador != '' ORDER BY identificador DESC LIMIT 6");
$ultimos_lotes = [];
...
<style>
    ...
    #traduccionBonTable { margin-bottom: 0 !important; width: 100% !important; }
    #traduccionBonTable thead th { background-color: #f8fafc; color: #475569; border-bottom: 2px solid #e2e8f0; font-size: 0.7rem; font-weight: 700; letter-spacing: 0.5px; padding: 6px 8px; text-transform: uppercase; }
    #traduccionBonTable tbody td { vertical-align: middle; font-size: 0.78rem; color: #334155; border-bottom: 1px solid #f1f5f9; padding: 3px 8px !important; line-height: 1.15; }
    ...
    /* MODALES MÓDULO */
    .modal-traduccion-bon { display: none; position: fixed; z-index: 1050; left: 0; top: 0; width: 100%; height: 100%; background-color: rgba(15, 23, 42, 0.4); backdrop-filter: blur(6px); padding-top: 6vh; }
    .modal-content-traduccion-bon { background-color: #ffffff; margin: auto; padding: 2rem; border: 1px solid rgba(255,255,255,0.2); width: 90%; max-width: 600px; border-radius: 20px; box-shadow: 0 25px 50px -12px rgba(0, 0, 0, 0.25); animation: modalFadeIn 0.3s cubic-bezier(0.16, 1, 0.3, 1); }
    ...
    .modal-content-traduccion-bon .form-control, .modal-content-traduccion-bon .form-select { border-radius: 8px; border: 1px solid #cbd5e1; padding: 0.6rem 1rem; font-size: 0.9rem; }
    .modal-content-traduccion-bon .form-control:focus, .modal-content-traduccion-bon .form-select:focus { border-color: #10b981; box-shadow: 0 0 0 3px rgba(16, 185, 129, 0.1); }
</style>
...
<div class="module-wrapper">
    ...
    <div class="page-header-inner d-flex flex-wrap justify-content-between align-items-center gap-3">
        <div>
            <h1 class="page-title-inner">Traducciones</h1>
            <p class="page-subtitle-inner"><i class="fas fa-gift me-1"></i> Módulo de Bonificación Académica</p>
        </div>
        
        <div class="d-flex gap-2 flex-wrap">
            <button id="openModaltrb" class="btn-modern btn-m-xls">
                <i class="fas fa-file-excel"></i> Exportar XLS
            </button>
            <button id="openModalCuadrostrb" class="btn-modern btn-m-cuadros">
                <i class="fas fa-table"></i> Generar Cuadros
            </button>
            <button id="openModalResolucionestrb" class="btn-modern btn-m-res">
    ...
    <div class="table-responsive">
        <table id="traduccionBonTable" class="table table-hover align-middle" data-order='[[ 0, "desc" ]]' style="width:100%">
            <thead>
                <tr> 
            ...
            <tbody>
                ...
                if ($result && $result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        ...
                        echo '<td><div class="text-truncate-custom" style="max-width: 180px;" title="' . htmlspecialchars($producto) . '">' . htmlspecialchars($producto) . '</div></td>';
                        echo '<td class="text-center">' . htmlspecialchars($row['numero_profesores']) . '</td>';
                        ...
                        echo '<a href="editar_traduccion_bon.php?id=' . $row['id'] . '" class="btn btn-light border btn-action text-primary shadow-sm" title="Editar"><i class="fas fa-pen"></i></a> ';
                        echo '<button class="btn btn-light border btn-action text-danger shadow-sm" onclick="confirmDeleteTradBon(' . $row['id'] . ')" title="Eliminar"><i class="fas fa-trash-alt"></i></button>';
                        echo '</td>';
                        echo '</tr>';
                    }
                }
    ...
    <div class="quick-audit-section">
        <div class="quick-audit-title">
            <i class="fas fa-folder-tree text-secondary me-1"></i> Auditoría Rápida de Lotes (Bonificación - Traducciones)
        </div>
        <div class="lotes-carousel">
            <?php foreach($ultimos_lotes as $lote): ?>
                <a href="auditoria_lote_bon.php?lote=<?php echo urlencode($lote); ?>" class="lote-card" title="Ver lote: <?php echo htmlspecialchars($lote); ?>">
                    <i class="fas fa-globe text-success"></i> <?php echo htmlspecialchars($lote); ?>
                </a>
            <?php endforeach; ?>
        </div>
    </div>
...
<div id="modaltrb" class="modal-traduccion-bon">
    <div class="modal-content-traduccion-bon">
        <div class="d-flex justify-content-between align-items-center mb-4">
             <h4 class="fw-bold text-success m-0"><i class="fas fa-file-excel me-2"></i>Reporte XLS (Bonificación)</h4>
             <span class="close-modal" onclick="$(this).closest('.modal-traduccion-bon').fadeOut(200);">&times;</span>
        </div>
        <form action="report_traduccion_bon.php" method="GET">
            <div class="mb-3">
                <label for="ano_xls_trb" class="form-label text-secondary fw-semibold">Año:</label>
                <select name="ano" id="ano_xls_trb" class="form-select">
                    <option value="">Todos los años...</option>
                    <?php foreach($unique_years as $y) echo "<option value='$y'>$y</option>"; ?>
                </select>
            </div>
            <div class="mb-4">
                <label for="identificador_solicitud_trb" class="form-label text-secondary fw-semibold">Identificador de Solicitud:</label>
                <select name="identificador_solicitud" id="identificador_solicitud_trb" class="form-select">
...
<div id="modalCuadrostrb" class="modal-traduccion-bon">
    <div class="modal-content-traduccion-bon">
        <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <form action="cuadros_traduccion_bon.php" method="GET">
            <div class="mb-3">
                <label for="ano_cuadros_trb" class="form-label text-secondary fw-semibold">Año:</label>
                <select name="cuadro_ano" id="ano_cuadros_trb" class="form-select">
                    <option value="">Todos los años...</option>
                    <?php foreach($unique_years as $y) echo "<option value='$y'>$y</option>"; ?>
                </select>
            </div>
            <div class="mb-4">
                <label for="cuadro_identificador_solicitud_trb" class="form-label text-secondary fw-semibold">Identificador de Solicitud:</label>
                <select name="cuadro_identificador_solicitud" id="cuadro_identificador_solicitud_trb" class="form-select">
                    <option value="">Todos los paquetes...</option>
                    <?php
                    foreach ($identificadores as $id) {
                        echo '<option value="' . htmlspecialchars($id) . '" data-ano="' . substr($id, 0, 4) . '">' . htmlspecialchars($id) . '</option>';
...
<div id="modalResolucionestrb" class="modal-traduccion-bon">
    <div class="modal-content-traduccion-bon" style="max-width: 650px;">
        <div class="d-flex justify-content-between align-items-center mb-4">
             <h4 class="fw-bold text-info m-0" style="color: #0284c7 !important;"><i class="fas fa-file-word me-2"></i>Resoluciones Bonificación</h4>
             <span class="close-modal" onclick="$(this).closest('.modal-traduccion-bon').fadeOut(200);">&times;</span>
        </div>
        <form action="resoluciones_traduccion_bon.php" method="GET">
            <div class="row bg-light p-3 mb-4 border rounded-3 mx-0">
                <div class="col-md-6 mb-2">
                    <label for="ano_res_trb" class="form-label text-secondary fw-semibold">Filtro por Año:</label>
                    <select id="ano_res_trb" class="form-select">
                        <option value="">Seleccione un año...</option>
                        <?php foreach($unique_years as $y) echo "<option value='$y'>$y</option>"; ?>
                    </select>
                </div>
                <div class="col-md-6 mb-2">
                    <label for="cuadro_identificador_trb" class="form-label text-secondary fw-semibold">Identificador (Paquete):</label>
                    <select name="cuadro_identificador_solicitud" id="cuadro_identificador_trb" class="form-select" required>
...
<script>
    ...
    (function() {
        function initTradBonModule() {
            
            // Función de filtro global
            window.applyYearFilterTradBon = function(yearSelectId, idSelectId) {
            ...
            // Anulación
            window.confirmDeleteTradBon = function(id) {
                if (confirm("¿Estás seguro de que quieres eliminar esta traducción?")) {
                    const motivo = prompt("Por favor, indique el motivo de la anulación:");
                    if (motivo && motivo.trim() !== "") {
                        window.location.href = 'eliminar_traduccion_bon.php?id=' + id + '&motivo=' + encodeURIComponent(motivo);
                    } else {
            ...
            $(document).ready(function() {
                // Control de Modales
                $('#openModaltrb').on('click', function() { $('#modaltrb').fadeIn(200); });
                $('#openModalCuadrostrb').on('click', function() { $('#modalCuadrostrb').fadeIn(200); });
                $('#openModalResolucionestrb').on('click', function() { $('#modalResolucionestrb').fadeIn(200); }); 
                
                // Cerrar haciendo clic fuera
                $(window).on('click', function(event) {
                    if ($(event.target).hasClass("modal-traduccion-bon")) {
                        $(event.target).fadeOut(200);
                    }
                });

                // Inicializar DataTables
                if (!$.fn.DataTable.isDataTable('#traduccionBonTable')) {
                ...
                // Eventos de filtro
                $('#ano_xls_trb').on('change', function() { applyYearFilterTradBon('ano_xls_trb', 'identificador_solicitud_trb'); });
                $('#ano_cuadros_trb').on('change', function() { applyYearFilterTradBon('ano_cuadros_trb', 'cuadro_identificador_solicitud_trb'); });
                $('#ano_res_trb').on('change', function() { applyYearFilterTradBon('ano_res_trb', 'cuadro_identificador_trb'); });
            });
        }
